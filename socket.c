...
static void sock_first_cb(EV_P_ ev_io *w, int revents)
{
    //printf("this cb was called\n");

    socket_t * sock = (socket_t *)w;

    sock->read_cb(sock, revents);
}

int sock_evstart(socket_t *s1, struct ev_loop * mainloop)
{
    printf("registering ev watcher\n");
    ev_io_init(
            &(s1->evio), 
            sock_first_cb, 
            s1->fd, 
            EV_READ);
    ev_io_start(mainloop, &(s1->evio));
    return 0;
}
...
int sock_connect(socket_t *sock, int is_serv)
{
    ...
    {
        int yes = 1;
        setsockopt(sock->fd, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int));

        if(debug_level >= DEBUG_LEVEL1)
        {
            printf("Listening on UDP %s\n",
                   sock_get_str(sock, addrstr, sizeof(addrstr)));
        }
...
void xor_round(char * data, int len)
{
    const char * key = "04fhqhoiuhbr9cp-8h31rn9p8qwh89qphacf9[qwe9a9h[-dcva9sdfhsdiuoh[0890q bhfupioandvfuhio";
